...
const MaintenanceFees = () => {
  ...
  const {
    refetch: refetchMaintenanceFees,
    loading: loadingMaintenanceFees,
    data: MaintenanceFeesData,
  } = useGet({
    url: `${apiUrl}/maintenance_feez/view_maintanence`,
  });
  const { postData, loadingPost, response } = usePost({
    url: `${apiUrl}/maintenance_feez/update/${selectedRow?.id}`,
  });
  ...
  const handleEdit = (Maintenance) => {
    const fullMaintenanceData = MaintenanceFeesData?.maintenance_fees.find(
      (o) => o.id === Maintenance.id,
    );
  ...
  const handleDeleteConfirm = async () => {
    const success = await deleteData(
      `${apiUrl}/maintenance_feez/delete/${selectedRow.id}`,
    ...
    if (success) {
      ...
      setMaintenanceFees(
        maintenanceFees.filter(
          (Maintenance) => Maintenance.id !== selectedRow.id,
        ),
      );
    }
  };
              ...
              <Tabs defaultValue="english" className="w-full">
                <TabsContent value="english">
                  <MaintenanceFormFields
                    ...
                    loading={loadingPost} // Use the correct loading state
                  />
                </TabsContent>
              </Tabs>
            </div>
